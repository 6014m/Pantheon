...
--     long -> heavy (the split is the "Heavy Length" slider).
...
--     attack and committed by its length. This is what makes brand-new styles get
--     picked up without hand-adding ids, and it can't be fooled by a dash/emote because
--     those never precede a hit.
--   * Anims you've already LOGGED (SUSPECT) are treated as known-combat anims and
--     classified by length the first time they play -- no need to eat a hit first.
...
-- Anim ids you've LOGGED (gakuran_new_anims.json) that aren't in the static DB. Treated
-- as SUSPECT known-combat anims: classified by their live length the first time they
-- play, so they start getting parried immediately -- "make sure we have em all".
local SEED_LOGGED = {
...
local SUSPECT = {}
for _, id in ipairs(SEED_LOGGED) do SUSPECT[id] = true end
...
----------------------------------------------------------------- optional exact-hit-frame (GetObjects)
-- Purely a timing refinement; classification NEVER depends on this. If GetObjects is
-- unavailable/blocked (common in executors), hitTimeCache[id]=false and we use hitFrac.
...
local function learnHitTime(id)
    if not (S.markerTiming and id) then return end
...
-- Schedule the timed block/dodge. Read Anim Speed reads the LIVE playhead for exact
-- time-to-impact (waits one frame so a play-then-AdjustSpeed has settled). If the game
-- left Speed ~= 1 (didn't speed the track up for a small rig), fold in the body-size
-- scale so a small attacker still gets a shorter windup. Falls back to fixed windup x
-- size when the track can't be read or the toggle's off.
local function scheduleReaction(plr, trk, id, kind, fallbackWindup, leadOffset, fire)
    local function debugLog(src, wait, extra)
        if S.debug then
            log.info(("[gakuran] time %s %s h=%.1f ss=%.2f %s wait=%.3f")
                :format(id, kind, stableHeight(plr), sizeScale(plr), extra or "", wait))
        end
    end
    local function fallback()
        local wait = math.max(fallbackWindup * sizeScale(plr) - leadOffset - ping(), 0)
        lastTiming = { src = "flat", sec = wait }
        debugLog("flat", wait, "flat")
    ...
    task.defer(function()
        ...
        if ok and sok and pok and type(len) == "number" and len > 0.05
           and type(spd) == "number" and spd > 0.01 and type(pos) == "number" then
            ...
            -- size differentiation when the game didn't scale Speed for a small rig
            ...
            debugLog("live", wait, ("spd=%.2f len=%.2f hitAt=%.2f"):format(spd, len, hitAt))
            task.delay(wait, fire)
        else
            fallback()
        end
...
local function react(attacker, id, trk)
    ...
    if not kind then
        ...
        -- Any unknown: stash as a candidate for the hit-confirmation learner.
        if not kind and inReach and not KNOWN[id] then
    ...
    learnHitTime(id)   -- optional exact-hit-frame warm-up (async, harmless if unavailable)
    ...
    if kind == "heavy" then
        ...
        if S.dodge then
            scheduleReaction(attacker, trk, id, kind, CFG.heavyWindup, CFG.dodgeLead, function()
                tapKey(DODGE_KEY); counts.dodge = counts.dodge + 1
            end)
        end
    elseif S.parry then
        scheduleReaction(attacker, trk, id, kind, CFG.windup, CFG.perfectLead, function()
...
local function bindSelf(c)
    ...
    track(hum.HealthChanged:Connect(function(h)
        if h < last - 0.01 then confirmFromDamage() end
        last = h
    end))
end
...
function GKN.register()
    ...
    subToggle(holder, 12, "Debug Timing",       "debug")

    components.Section(holder, "Tuning").LayoutOrder = 20
    tuneSlider(holder, 21, "Anim Hit Point", "hitFrac",   0.20, 0.90, 0.05)
    tuneSlider(holder, 22, "Heavy Length (M1/M2 split)", "heavyLen", 0.40, 1.50, 0.05)
    ...
    track(RS.Heartbeat:Connect(function(dt)
        ...
        statusLbl.Text = ("nearest %s | x%.1f | t:%s %.2fs | P%d D%d B%d | learned:%d%s"):format(
            hStr, REACH, lastTiming.src, lastTiming.sec, counts.parry, counts.dodge, counts.gbreak, counts.learned, carryStr)
    end))
