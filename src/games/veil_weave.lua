...
local JUMP_ATTACKS = {
    ...
    -- weaveFallback: it lands only ~0.38 s in; if your own M1 swing is still locking jumps
    -- (user: "my m1's stop double jumps") the jump can't make it, so it's woven instead
    ["81155999312581"] = { impacts = { 0.38 }, range = 55, name = "Smelter Demon horizontal swing", weaveFallback = true },
}
...
local NEVER_LEARN = { ["110431028319368"] = true, ["111237192632620"] = true }
...
local function plan(t)
    ...
    -- a quick jump attack while your M1 still locks jumping: weave it instead (if allowed)
    for _, h in ipairs(open) do
        if h.jump and h.weaveFallback and h.t - t <= JUMP.lead + 0.02 then
            local c = LP.Character
            local hum = c and c:FindFirstChildOfClass("Humanoid")
            local jumpedFor = false
            for _, p in ipairs(jumps) do if jumpCovers(p, h) then jumpedFor = true end end
            if hum and not jumpedFor and not canJump(hum) then
                h.jump = false
                dlog("JUMPBLOCKED %s -> weave instead (your swing locks jumping)", h.reason)
            end
        end
    end
    for _, h in ipairs(open) do
    ...
    if jumpAtk then
        ...
        if r0 and mroot.Parent and (mroot.Position - r0.Position).Magnitude <= jumpAtk.range then
            for i, dt in ipairs(jumpAtk.impacts) do
                ...
                impacts[#impacts].weaveFallback = jumpAtk.weaveFallback
            end
        end
        return
    end
