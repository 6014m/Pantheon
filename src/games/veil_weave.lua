...
local function stamina()
    local c = LP.Character
    local v = c and c:FindFirstChild("Stamina")
    return (v and v:IsA("ValueBase")) and (tonumber(v.Value) or 0) or 0
end
...
-- to it) for the press.
local function dashKeyFor(threat)
    for _, k in ipairs(MOVE_KEYS) do
        if UIS:IsKeyDown(k) then return nil end
    end
    if CFG.dashDir == "Where you're moving only" then return nil end
    local r = root()
    ...
    if CFG.dashDir == "Sideways" then away = Vector3.new(-away.Z, 0, away.X) end
...
local function sendDash(threat)
    local dir = dashKeyFor(threat)
    lastInject = now()
    pcall(function()
...
local function plan(t)
    ...
    if attempt then
        ...
        elseif t - attempt.lastPress >= RETRY then
            attempt.lastPress = t
            if attempt.dash then sendDash(attempt.from) else sendKey() end
        end
        return
    end
    local open = {}
...
local function deathBlastReach(name)
    if string.find(name, "Dissonant Brute", 1, true) then return 19 end
    if string.find(name, "Bloated Hiveling", 1, true) or string.find(name, "Dissonant", 1, true) then return 11 end
    return nil
end
...
local function onMobDied(model, mroot)
    ...
    impacts[#impacts + 1] = { t = now() + DASH.lead, reason = model.Name .. " death blast", kind = "melee",
...
function Weave.feature()
    return {
        ...
        settings = {
            ...
            { type = "slider", name = "Keep stamina for yourself", key = "dash_reserve", min = 0, max = 100, step = 5,
