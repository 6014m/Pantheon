...
local CHANNELS = {
    ...
    -- Festering Wound ground strikes 127293443282395 (4.6 s): TripleSmashEffects shockwave,
    -- 36.75 dmg strikes at ~0.96 and ~1.41 s (user: get away or lose ~90% HP). Dash away at
    -- once; the strikes are also queued as weaves in case they still reach you.
    ["127293443282395"] = { name = "Festering Wound ground strikes", range = 45, strikes = { 0.96, 1.41 } },
}

-- JUMP_ATTACKS: undashable + unweavable -> be in the air when it lands (double jump).
-- Festering Wound slam-down 80778819711637: the shockwave (TripleSmashEffects, 85 wide)
-- goes out ~1.2 s after this anim (its wind-up 106952798050011 plays ~2 s before). His
-- root never leaves the ground, so it's timed from the anim, not from a fall.
local JUMP_ATTACKS = {
    ["80778819711637"] = { impact = 1.2, range = 60, name = "Festering Wound slam" },
}
...
local function onMobAnim(model, mroot, track)
    ...
    if jumpAtk then
        ...
        if r0 and mroot.Parent and (mroot.Position - r0.Position).Magnitude <= jumpAtk.range then
            want(now() + jumpAtk.impact, jumpAtk.name, "melee", mroot.Position, "jump:" .. id)
