...
-- Festering Wound slam-down 127293443282395 (user: he goes up invisible and slams down):
-- its AIHold attribute is on for exactly this anim (4.65 s); Indicator sound +
-- TripleSmashEffects at 0 s, Grunt/Smash sounds ~0.83 s, damage 36.75 at ~0.96 and ~1.41 s
-- (recorded 4x). His root never leaves the ground (the flight is visual), so it's timed
-- from the anim. One double jump covers both hits (airborne ~0.15-0.85 s after the tap).
local JUMP_ATTACKS = {
    ["127293443282395"] = { impacts = { 0.96, 1.41 }, range = 90, name = "Festering Wound slam-down" },
}
