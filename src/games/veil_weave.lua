...
local pending = {}         -- planned weaves: { at, first, last, reason } (first/last = impacts it covers)
...
local function confirmWeave(t)
    ...
    if attempt then
        weaves += 1
        if CFG.verbose then
            log.info(string.format("[Weave] #%d %s (%.0f ms after first press)", weaves, attempt.plan.reason,
...
local function lastWeave()
    local last = done[#done] or -math.huge
    if attempt then last = math.max(last, now()) end
    for _, s in ipairs(pending) do last = math.max(last, s.at) end
    return last
end

-- Plan a weave for a hit expected at `impact` (os.clock time).
-- One weave dodges every hit that lands while it's active (the game batches them), so a
-- new hit first tries to ride along with a weave that's done or planned; a hit that would
-- land in the cooldown after a weave can't be dodged at all, so it's dropped.
local function want(impact, reason)
    for _, p in ipairs(done) do if covers(p, impact) then return end end
    if attempt and impact - now() >= CFG.iframeFrom and impact - now() <= CFG.iframeTo then return end
    for _, s in ipairs(pending) do
        local first, last = math.min(s.first, impact), math.max(s.last, impact)
        local lo = math.max(last - CFG.iframeTo, now())
        local hi = first - CFG.iframeFrom
        if lo <= hi then
            s.first, s.last = first, last
            s.at = math.clamp((first + last) / 2 - leadNow(), lo, hi)
            s.reason = s.reason .. " + " .. reason
            return
        end
    end
    local at = impact - leadNow()
    local prev = lastWeave()
    if at < prev + CFG.cooldown then
        at = prev + CFG.cooldown
        if impact - at < CFG.iframeFrom then
            if CFG.verbose then log.info("[Weave] can't cover (cooldown): " .. reason) end
            return
        end
    end
    pending[#pending + 1] = { at = math.max(at, now()), first = impact, last = impact, reason = reason }
...
local function pump(t)
    if attempt then
        if t > attempt.deadline then
            if CFG.verbose then log.info("[Weave] game refused every press (busy / cooldown): " .. attempt.plan.reason) end
            attempt = nil
    ...
    local s = pending[1]
    if s and t >= s.at then
        table.remove(pending, 1)
        if UIS:GetFocusedTextBox() or not alive() then return end
        attempt = { plan = s, started = t, lastPress = t, deadline = s.first - CFG.iframeFrom }
        sendKey()
