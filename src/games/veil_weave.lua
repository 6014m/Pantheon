...
local function isFriendlyModel(model)
    return Players:GetPlayerFromCharacter(model) ~= nil or isSummon(model)
end

-- nearest mob vs nearest player/summon to a point
local function closerToFriendly(pos)
    local mobD, friendD = math.huge, math.huge
    for _, pl in ipairs(Players:GetPlayers()) do
        local c = pl.Character
        local r = c and c:FindFirstChild("HumanoidRootPart")
        if r then friendD = math.min(friendD, (r.Position - pos).Magnitude) end
    end
    local folder = Workspace:FindFirstChild("Monsters")
    if folder then
        for _, m in ipairs(folder:GetChildren()) do
            ...
            if r then
                ...
                if isSummon(m) then friendD = math.min(friendD, d) else mobD = math.min(mobD, d) end
            end
        end
    end
    return friendD < mobD
...
local function onPart(part)
    ...
    -- anything inside a player's character or a summon is friendly; inside a mob only
    -- named hostile parts count (limbs / accessories are not attacks)
